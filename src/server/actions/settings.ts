...
const schema = z.object({
  ...
  gforceImageUrl: z.string().optional(),
  gforceImageWidth: z.coerce.number().int().min(160).max(900).default(480),
...
export async function updateSettings(
  ...
  const parsed = schema.safeParse({
    ...
    gforceImageUrl: formData.get("gforceImageUrl") ?? undefined,
    gforceImageWidth: formData.get("gforceImageWidth") ?? 480,
  ...
  const data = {
    ...
    faviconUrl: nullIfEmpty(formData.get("faviconUrl")),
    gforceImageUrl: nullIfEmpty(formData.get("gforceImageUrl")),
    gforceImageWidth: parsed.data.gforceImageWidth,
